# 🤖 Enhanced Multi-Format Document Chat Assistant

A powerful Retrieval-Augmented Generation (RAG) system that allows you to chat with your documents in multiple formats through various interfaces. Uses **Ollama** for local AI inference and **FAISS** for fast document retrieval with intelligent response synthesis.

## ✨ Key Features

- **📄 Multi-Format Support**: PDF, DOCX, DOC, XLSX, XLS, PPTX, PPT documents
- **🔍 Smart Retrieval**: FAISS-powered semantic search with configurable context
- **🧠 Local AI**: Powered by Ollama for private, offline AI responses  
- **💬 Multiple Interfaces**: Streamlit web UI, Flask API, and command line
- **📚 Source Tracking**: Exact page numbers and document source references
- **🎯 Intelligent Responses**: Synthesizes information instead of just citing sources
- **💡 Crisp Answers**: Direct yes/no responses and immediate general chat handling
- **📁 Recursive Scanning**: Processes documents in all subdirectories
- **🌐 Network Ready**: LAN access for team collaboration
- **⚡ Performance Optimized**: Model caching and fallback modes
...
git clone https://github.com/dantuswaroop/ChatBot-RAG.git
cd ChatBot-RAG
...
Place your documents in the `data/` directory (supports any folder structure):

```bash
# Examples of supported files:
cp your_document.pdf data/
cp report.docx data/reports/
cp spreadsheet.xlsx data/analysis/
cp presentation.pptx data/meetings/
```
...
# This will:
# - Extract text from all supported formats in data/
...
```bash
# Quick start with optimized loading and LAN access
...
python src/app.py --ask "What are the main security requirements?"

# Start interactive terminal chat
python src/app.py --chat

# Example questions:
python src/app.py --ask "Does the app support streaming?"
python src/app.py --ask "Hello, how are you?"
```
...
#### 1. **General Chat Handling**
```bash
# These are handled immediately without document search:
"Hello, how are you?"
"Hi there!"
"What's your name?"
"Can you help me?"
```
**Response:** Friendly chat responses without accessing documents.

#### 2. **Crisp Yes/No Answers**
```bash
# Questions starting with these get direct yes/no responses:
"Does the app support streaming?"
"Can users create personal programs?"
"Is authentication required?"
```
**Response:** Starts with **"Yes"** or **"No"** followed by supporting details.

#### 3. **Comprehensive Analysis**
```bash
# Complex questions get detailed, synthesized responses:
"What are the main features of the D-chipset app?"
"Explain the equalizer feature and how it works"
"What are the security requirements for the system?"
```
**Response:** Analytical, organized information with key insights and implications.
