...
def initialize_session_state():
    """Initi            # Display the formatted response
            st.markdown(response)
            
            # Add interactive source explorer
            if relevant_chunks:
                display_source_explorer(relevant_chunks) variables"""
...
def get_answer(query: str, top_k: int = 5):
    """Get answer from the RAG system with nicely formatted output"""
    try:
        ...
        formatted_response = format_structured_response(query, answer, relevant_chunks)
...
def format_structured_response(query: str, answer: str, relevant_chunks: list) -> str:
    """Format the response into structured categories"""
...
---
*Generated by RAG PDF Chat Assistant*
"""
...
def display_source_explorer(relevant_chunks):
    ...
    with col2:
        ...
        with st.container():
            ...
            st.metric("Chunks Retrieved", len(relevant_chunks))
            ...
            for source, chunks in unique_sources.items():
                pages_set = set()
                for chunk in chunks:
                    pages_set.update(chunk.get('pages', []))
                
                st.write(f"**{source}**")
                st.write(f"• {len(chunks)} excerpts")
                st.write(f"• {len(pages_set)} pages")
                st.write("")
...
def main():
    ...
    if sample_question:
        ...
        with st.chat_message("assistant"):
            ...
            # Add interactive source explorer (same as below)
            if relevant_chunks:
